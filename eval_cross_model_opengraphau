...
class OpenGraphAUEvaluator:
    ...
    def score_batch_shared(self, x_batch: torch.Tensor) -> torch.Tensor:
        return self.model.score_batch_shared_logits(x_batch).detach().float().cpu()
...
def evaluate_subset_cross_model(
    ...
    for batch in loader:
        ...
        s_clean = og_eval.score_batch_shared(x)
        s_adv = og_eval.score_batch_shared(x_adv)
        s_rand = og_eval.score_batch_shared(x_rand)
...
def parse_args():
    ...
    ap.add_argument("--model_path", type=str, default="", help="Unused in self-attack mode; kept only for backward CLI compatibility")
...
def main():
    ...
    print(f"Self-attack shared AUs: {shared_aus}")
    ...
    summary_out = {
        "protocol": "self_attack_opengraphau_to_opengraphau",
        "shared_aus": shared_aus,
        "states": STATE_LABELS,
